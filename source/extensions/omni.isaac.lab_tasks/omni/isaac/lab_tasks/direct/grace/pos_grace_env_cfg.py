    ...
    triangle_visualizer : VisualizationMarkersCfg = VisualizationMarkersCfg(
        ...
        markers={
            ...
            "a_gi": sim_utils.UsdFileCfg( # taken from GREEN_ARROW_X_MARKER_CFG
                ...
                scale=(0.2, 0.2, 1.),
                visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.8, 0.2, 0.2)), #agi rossa
            ),
            "gravity": sim_utils.UsdFileCfg(
                usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/UIElements/arrow_x.usd",
                scale=(0.2, 0.2, 1.),
                visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.1, 0.9, 0.1)), #gravity verde
            ),
            "ag_total": sim_utils.UsdFileCfg(
                usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/UIElements/arrow_x.usd",
                scale=(0.2, 0.2, 1.),
                visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.2, 0.2, 0.8)), #ag blu
            ),
            "a_gilim": sim_utils.UsdFileCfg(
                usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/UIElements/arrow_x.usd",
                scale=(0.2, 0.2, 1.),
                visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(1., 1., 1.)), #agilim bianca
            )
        }
    )

# PER CHECKPOINT SENZA VACUUM
...
#     a_marg_reward_scale             = 0.001 #0.001
#     vacuum_action_rate_reward_scale = -0.0
#     adhesion_quality                = 0.0
#     climbing_progress               = 0.0
#     stability                       = 0.0

    # PER CHECKPOINT CON VACUUM
    position_tracking_reward_scale = 10.0
    heading_tracking_reward_scale = 5.0
    joint_vel_reward_scale= -0.001
    joint_torque_reward_scale= -1.0e-05
    joint_vel_limit_reward_scale= -1.0
    joint_torque_limit_reward_scale= -0.2
    base_acc_reward_scale= -0.001
    base_lin_acc_weight= 1.0
    base_ang_acc_weight= 0.02
    feet_acc_reward_scale= -0.002
    action_rate_reward_scale= -0.01
    max_feet_contact_force= 700
    feet_contact_force_reward_scale= -1.0e-05
    wait_time= 0.2
    dont_wait_reward_scale= -1.0
    move_in_direction_reward_scale= 1.0
    stand_min_dist= 0.25
    stand_min_ang= 0.5
    stand_at_target_reward_scale= -0.5
    ...
    theta_marg_sum_reward_scale= 0.00036741392464462934
    a_marg_reward_scale= 0.01
    vacuum_action_rate_reward_scale= -0.001
    adhesion_quality= 0.1
    climbing_progress= 0.1
    stability= 0.01

    # PER TUTTI I CHECKPOINT
    vacuum_efficiency= 0.01
    vacuum_quality_scale: float = 0
    terrain_adaptation_gain: float = 0
    pca_min_contacts: int = 0
    stability_core_scale: float = 0.0
    terrain_adaptation_scale: float = 0.0
